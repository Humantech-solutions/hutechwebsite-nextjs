...
const MEDIA_LOADER_JSON =
  "https://cdn.builder.io/o/assets%2F4975bfe7ef7b41d5ae516629e61e5b7d%2F36fc6adaf5be478ba00109167e63b90f?alt=media&token=fc64a9ff-758d-4faf-a9f2-ee380b2ca89c&apiKey=4975bfe7ef7b41d5ae516629e61e5b7d";
...
const LoadingAnimation: React.FC<{
  ...
  useEffect(() => {
    ...
    const url =
      src ||
      "https://cdn.builder.io/o/assets%2F53d4e9f16f794d438e3fbb8ee6709fb8%2F71e792acb32c41d8b2390e222e9d80f8?alt=media&token=1dc9711e-aec0-4e92-9171-4c6922ab704c&apiKey=53d4e9f16f794d438e3fbb8ee6709fb8";
  ...
  return (
    <div className={className || "chat-loader-inline"} aria-hidden="true">
      <div
        ref={containerRef}
        className={`lottie-loader ${direction === "right" ? "lottie-direction-right" : ""}`}
        role="status"
        aria-label="Loading"
      />
    </div>
...
const ERROR_GIFS: Record<ErrorKind, string> = {
  ...
  http_5xx:
    "https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExZGljYnpscmlodGs2N3VrOG1ob2kxbDlldnJvNHY5bmZ4d3ZmMmU5aiZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/0Dxuk20eU0bDYaxRR8/giphy.gif",
  json_parse_error:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExd3g0NHVmYXBwNXowb3MzMngyeGJvN2t3cTZ1YWR4OHBhYndpcWkwZyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/nIlgTxY29wJuU/giphy.gif",
  unknown_error:
    "https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeHR6dW01M2xoZGp5M3hkdzhiZHg3M2xueTM5dDV2aGFta2JxZmllNCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/XftgiBWR1ya5FswTSE/giphy.gif",
};
...
function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const restored: Message[] = arr.map((m: any) => ({
          ...m,
  ...
  useEffect(() => {
    if (!hydrated) return;
    try {
      localStorage.setItem("__page", currentPage);
    } catch {}
  ...
  const validateBotResponse = (response: Partial<BotResponse>): BotResponse => {
    const isImageUrl = (u: string) =>
      /^(data:image\/.+)|\.(png|jpe?g|gif|webp|svg|bmp|tiff?|heic|avif)(\?|#|$)/i.test(u || "");
    ...
    return {
      ...
      file_links: Array.isArray(response.file_links)
        ? response.file_links.filter(
            (item) =>
              item &&
              typeof item.title === "string" &&
              typeof item.url === "string" &&
              (/\.(pdf|docx|xlsx|pptx|zip)(\?|#|$)/i.test(item.url) ||
                /\/(download|files?|documents?|pdf)\//i.test(item.url))
          )
        : undefined,
      tables: Array.isArray(response.tables)
        ? response.tables.filter(
            (table) =>
              table &&
  ...
  const parseJsonResponse = (jsonData: any): BotResponse => {
    ...
    if (jsonData.response && typeof jsonData.response === "object") {
      const rawResponse = {
        ...
        file_links:
          jsonData.response.file_links || jsonData.response.fileLinks || jsonData.response.files,
  ...
  const clearChat = () => {
    ...
    setMessages(reset);
    try {
      const toSave = reset.map((m) => ({ ...m, timestamp: (m.timestamp as Date).toISOString() }));
      localStorage.setItem("__chatMsgs", JSON.stringify(toSave));
    } catch {}
    try {
      localStorage.removeItem("__contactNoteShown");
    } catch {}
    setInputValue("");
    setCurrentPage("chat");
    setShowMenu(false);
  ...
  // Chat Page
  return (
    <div className="body bg-white" id="body">
      {/* Chat History Panel */}
      ...
      <div className="chat-input-sticky sticky bottom-0 bg-white py-4">
            ...
            <div className="chat-input-wrapper" ref={menuRef}>
              ...
              >
                <svg
                  ...
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  className="chat-clear-icon h-5 w-5"
                >
                  <path
                    strokeLinecap="round"
...
const UserMessage: React.FC<{ text: string }> = ({ text }) => {
  return (
    <div className="flex justify-end">
      <div className="chat-bubble-user prose max-w-lg rounded-xl rounded-br-none p-4 text-sm shadow-md">
...
const MessageActions: React.FC<{
  ...
  const buildPlainAnswer = (): string => {
    const htmlAnswer = safeRenderMarkdown(
      renderIcons(renderTables(message.text || "", message.response?.tables || []))
    );
    let html = htmlAnswer
  ...
  const buildTablesPlain = (): string => {
    ...
    tbls.forEach((tbl) => {
      ...
      if (Array.isArray(tbl.headers) && tbl.headers.length)
        rows.push(tbl.headers.map((v) => String(v ?? "")));
  ...
  const handleCopy = () => {
    ...
    const answerText = (div.textContent || "")
      .replace(/\u00A0/g, " ")
      .split("\n")
      .map((l) => l.trimEnd())
      .join("\n")
    ...
    copyToClipboard(textToCopy)
      ...
      .catch((err) => {
        ...
        alert(
          `Copy failed due to browser restrictions. Please manually copy this content:\n\n${textToCopy}`
        );
      });
  };
  ...
  const generatePDF = async () => {
    if (!ENABLE_PDF_EXPORT) {
      alert("PDF export is temporarily disabled.");
      return;
    }
    try {
      ...
      const logoUrl =
        "https://cdn.builder.io/api/v1/image/assets%2Fdc4265ec2f2449c79371971938c19898%2F496df066ecdc418c994b8726633ae9a3?format=png&width=800";
      try {
      ...
      plain
        ...
        .map((p) => p.trim())
        .filter(Boolean)
        .forEach((p) => addParagraph(p));

      // Extract only images that appear in the answer content
      const answerHtml = safeRenderMarkdown(
        renderIcons(renderTables(message.text || "", message.response?.tables || []))
      );
      const htmlImgs = Array.from(answerHtml.matchAll(/<img[^>]+src=["']([^"']+)["']/gi)).map(
        (m) => m[1]
      );
      const mdImgs = Array.from(
        (message.text || "").matchAll(/!\[[^\]]*\]\(([^)\s]+)(?:\s+\"[^\"]*\")?\)/g)
      ).map((m) => m[1]);
      ...
      const drawTable = (tbl: Table) => {
        ...
        const headers =
          tbl.headers && tbl.headers.length
            ? tbl.headers
            : Array(tbl.rows[0]?.length || 1).fill("");
        const rows = tbl.rows || [];
      ...
      if (files.length) {
        y += 16;
        addParagraph("Files");
        files.forEach((f) => addParagraph(`- ${f.title}: ${f.url}`));
      }
      const pages = (message.response?.related_content || []).filter((p: any) => !p.image);
      if (pages.length) {
        y += 16;
        addParagraph("Related Pages");
        pages.forEach((p: any) => addParagraph(`- ${p.title}: ${p.url}`));
      }
  ...
  const generateMarkdown = async () => {
    if (!ENABLE_MD_EXPORT) {
      alert("Markdown export is temporarily disabled.");
      return;
    }
    try {
      const tablesToMarkdown = (tbl: Table): string => {
        const header = tbl.headers?.length
    ...
    try {
      ...
      try {
        ...
        children.push(
          new Paragraph({
            children: [
              new ImageRun({ data: bytes, transformation: { width: 360, height: 120 } } as any),
            ],
          })
        );
      ...
      paras.forEach((p) => {
        children.push(
          new Paragraph({ children: [new TextRun({ text: p })], spacing: { after: 100 } })
        );
      });

      const imgItems = (message.response?.related_content || []).filter((i) => i.image) as {
        image: string;
        title?: string;
      }[];
      for (const it of imgItems) {
        try {
          ...
          children.push(
            new Paragraph({
              children: [
                new ImageRun({ data: bytes, transformation: { width: w, height: h } } as any),
              ],
              spacing: { after: 120 },
            })
          );
      ...
      if (files.length) {
        ...
        files.forEach((f) => {
          children.push(
            new Paragraph({
              bullet: { level: 0 },
              children: [new TextRun({ text: `${f.title}: ${f.url}` })],
            })
          );
        });
      }
      ...
      if (pages.length) {
        children.push(
          new Paragraph({
            spacing: { before: 200 },
            children: [new TextRun({ text: "Related Pages", bold: true })],
          })
        );
        pages.forEach((p) => {
          children.push(
            new Paragraph({
              bullet: { level: 0 },
              children: [new TextRun({ text: `${p.title}: ${p.url}` })],
            })
          );
        });
      }
    ...
    <div className="message-actions flex items-center justify-between px-4 py-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="action-button flex items-center rounded-md p-2 text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-800"
          title="Share"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
                ...
                <button className="share-option" onClick={handleShareX} title="X">
                  <span className="share-icon x">
                    <svg
                      ...
                      height="24"
                    >
                      <path d="M17.53 2H20l-5.5 6.3L21 22h-6.4l-3.9-6.1L5.9 22H3.4l6-6.9L3 2h6.5l3.5 5.6L17.5 2h.03zM8.2 3.7h-1l8.8 14h1L8.2 3.7z" />
                    </svg>
                  </span>
...
const BotMessage: React.FC<{
  ...
  const answerImages = useMemo(() => {
    ...
    Array.from(html.matchAll(/<img[^>]+(?:src|data-src)=["']([^"']+)["'][^>]*>/gi)).forEach((m) =>
      collect.add(m[1])
    );
    ...
    Array.from(
      (message.text || "").matchAll(/!\[[^\]]*\]\(([^)\s]+)(?:\s+\"[^\"]*\")?\)/g)
    ).forEach((m) => collect.add(m[1]));
    // Anchor links to images
    Array.from(
      html.matchAll(
        /<a[^>]+href=["']([^"']+\.(?:png|jpe?g|gif|webp|svg|bmp|tiff?|heic|avif))(?:\?[^"']*)?["'][^>]*>/gi
      )
    ).forEach((m) => collect.add(m[1]));
  ...
  useEffect(() => {
    ...
    const done = () => {
      if (settled) return;
      finished += 1;
      if (finished >= answerImages.length) {
        settled = true;
        setImagesReady(true);
      }
    };
    answerImages.forEach((src) => {
      try {
        const im = new Image();
        im.onload = done;
        im.onerror = done;
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const fallback =
      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="64" height="24" viewBox="0 0 64 24"><rect width="64" height="24" fill="%23f3f4f6"/><text x="8" y="16" font-size="12" fill="%236b7280">Image</text></svg>';
...
const RelatedContentCarousel: React.FC<{ items: RelatedContent[] }> = ({ items }) => {
  ...
  const safeHostname = (websiteUrl: string): string => {
    try {
      const h = new URL(websiteUrl).hostname || "";
  ...
  return (
    <div className="mb-6 w-full">
      <h5 className="mb-2 px-4 font-semibold text-gray-800">Related content</h5>
      <div className="related-content-carousel-wrapper">
        ...
        <div ref={containerRef} className="related-content-horizontal-container">
            ...
            >
              <div className="mini-card-favicon">
                ...
                <div className="favicon-fallback" style={{ display: "none" }}>
                  ...
                  >
                    <path d="M12 2a10 10 0 100 20 10 10 0 000-20zm6.93 6h-2.95a15.65 15.65 0 00-1.38-3.56A8.03 8.03 0 0118.93 8zM12 4.04c.83 1.2 1.48 2.59 1.91 3.96h-3.82c.43-1.37 1.08-2.76 1.91-3.96zM4.26 14a7.82 7.82 0 010-4h3.38c-.08.66-.14 1.32-.14 2s.06 1.34.14 2H4.26zm.81 2h2.95c.32 1.3.8 2.51 1.38 3.56A8.03 8.03 0 015.07 16zm2.95-8H5.07a8.03 8.03 0 013.84-3.56C8.33 5.49 7.85 6.7 7.53 8zM12 19.96c-.83-1.2-1.48-2.59-1.91-3.96h3.82c-.43 1.37-1.08 2.76-1.91 3.96zM14.34 14H9.66c-.09-.65-.16-1.32-.16-2s.07-1.35.16-2h4.68c.09.65.16 1.32.16 2s-.07 1.35-.16 2zm1.13 5.56c.58-1.05 1.06-2.26 1.38-3.56h2.95a8.03 8.03 0 01-3.84 3.56zM16.36 14c.08-.66.14-1.32.14-2s-.06-1.34-.14-2h3.38a7.82 7.82 0 010 4h-3.38z" />
                  </svg>
                </div>
              </div>
              <div className="mini-card-content">
                <div className="mini-card-hostname">{safeHostname(item.url) || item.url}</div>
        ...
        {validImages.map((src, i) => (
          <div key={`${src}-${i}`} className="answer-image-card" style={{ position: "relative" }}>
            {!loaded[src] && (
              ...
              >
                <LoadingAnimation
                  className="chat-loader-inline media-loader"
                  direction="left"
                  src={MEDIA_LOADER_JSON}
                />
              </div>
            )}
            <img
...
const AnswerGifSmart: React.FC<{
  ...
  useEffect(() => {
    ...
    const text =
      `${base} ${(related || []).map((r) => `${r.title} ${r.url}`).join(" ")}`.toLowerCase();
    const intentText = (query || "").toLowerCase();

    const isLocation =
      /(\boffice|offices|location|address|headquarters|\bhq\b|branch|where|map|directions|route|campus|city|country|state|bengaluru|bangalore|delhi|mumbai|london|new york)/i.test(
        intentText || text
      );
    const isSuccess =
      /(success|successful|booked|booking|confirmed|confirmation|done|completed|achieved|congrats|congratulations|win|victory|approved)/i.test(
        intentText || text
      );
    const isServices =
    ...
    const hasBranding = (s?: string) => {
      ...
      if (!t) return false;
      if (/(logo|watermark|brand|branding|sponsor|sponsored|copyright|trademark|tm)/.test(t))
        return true;
      try {
        const u = new URL(raw);
    ...
    const extractSubject = (
      ...
      if (/(payroll|salary|salaries|compensation|payslip|pay slip|payment|invoice)/.test(s))
        return "payroll";
      if (/(leave|vacation|holiday|pto|time off)/.test(s)) return "leave";
      if (/(office|location|address|headquarters|hq|branch|map|route|pin|directions)/.test(s))
        return "office";
      if (/(^|\b)(hr|human resources)\b|policy|help|support/.test(s)) return "hr";
      if (/(meeting|calendar|schedule|appointment|call|conference|consult|consultation)/.test(s))
        return "meeting";
      if (/(contact|phone|email|reach|connect|get in touch|call us)/.test(s)) return "contact";
      // CEO / leadership queries
      if (
        /(^|\b)(ceo|founder|chairman|managing director|md|executive|leadership|leader|chief executive)\b/.test(
          s
        )
      )
        return "about";
    ...
    const scoreMeta = (meta: string, url: string, cat: Cat) => {
      ...
      if (
        /(glitch|distort|noise|static|grain|datamosh|matrix|error|404|bug|broken|corrupt|scanline|hacker|hack|leak)/.test(
          meta
        )
      )
        score -= 80;
      if (noisyOrCasual.some((k) => meta.includes(k))) score -= 60;
      catTokens[cat].forEach((k) => {
        if (meta.includes(k)) score += 3;
      });
      if (
        /(professional|clean|minimal|icon|illustration|diagram|flat|outline|line|animation|loop)/.test(
          meta
        )
      )
        score += 8;
      if (/(abstract|pattern|texture|kaleidoscope|psychedelic|trippy|glitter|neon)/.test(meta))
        score -= 35;
    ...
    const wantsCharts =
      /(analytics|statistic|statistics|chart|graph|dashboard|metrics|kpi|report|reporting|data visualization|visualization)/.test(
        intentText || text
      );
    const hasCharts = (s?: string) =>
      /\b(graph|chart|plot|scatter|bar|line chart|pie chart|histogram|axis|3d|mesh|surface|contour)\b/i.test(
        s || ""
      );
    ...
    const tryTenor = async (
      q: string
    ): Promise<{ url?: string; link?: string; title?: string } | null> => {
      if (!tenorkey) return null;
      try {
        const r = await fetch(
          `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent(q)}&key=${tenorkey}&limit=50&contentfilter=high&media_filter=gif,tinygif`
        );
        ...
        const ranked = items
          .map((it: any) => {
            ...
            const meta =
              `${it?.content_description || ""} ${it?.h1_title || ""} ${(it?.tags || []).join(" ") || ""} ${it?.itemurl || ""}`.toLowerCase();
            const score = scoreMeta(meta, url, category);
            return {
              url,
              link: it?.itemurl as string | undefined,
              title: it?.content_description || it?.h1_title || "gif",
              score,
              meta,
            };
          })
          .filter(
            (x: any) =>
              ...
              /\.gif(\?|$)/i.test(x.url) &&
              x.score >= MIN_SCORE &&
              matchesRequired(x.meta, subject as string) &&
              !hasBranding(x.meta) &&
              !hasBranding(x.link || "") &&
              (wantsCharts || !hasCharts(x.meta))
          )
    ...
    const tryGiphy = async (
      q: string
    ): Promise<{ url?: string; link?: string; title?: string } | null> => {
      if (!giphykey) return null;
      try {
        const r = await fetch(
          `https://api.giphy.com/v1/gifs/search?api_key=${giphykey}&q=${encodeURIComponent(q)}&limit=50&rating=g&lang=en`
        );
        const j = await r.json();
        ...
        const ranked = items
          .map((it: any) => {
            ...
            return {
              url,
              link: it?.url as string | undefined,
              title: it?.title || "gif",
              score,
              meta,
            };
    ...
    const searchBoth = async (queries: string[]) => {
      ...
      for (const raw of queries) {
        ...
        if (!cancelled && t1?.url) {
          setGif({ url: t1.url!, link: t1.link, title: t1.title });
          return true;
        }
        const g1 = await tryGiphy(q);
        if (!cancelled && g1?.url) {
          setGif({ url: g1.url!, link: g1.link, title: g1.title });
          return true;
        }
      }
      return false;
    ...
    const extractKeywords = (t: string): string[] => {
      ...
      const words = Object.keys(counts).sort(
        (a, b) => counts[b] - counts[a] || b.length - a.length
      );
    ...
    const allOverrides: GifOverridesMap = {
      ...
      ...Object.fromEntries(
        Object.entries(gifOverrides || {}).map(([k, v]) => [k.toLowerCase(), v])
      ),
      ...parseEnvOverrides(),
    };
    ...
    const getRotation = (): Record<string, number> => {
      try {
        return JSON.parse(localStorage.getItem("__gifRotation") || "{}") as Record<string, number>;
      } catch {
        return {};
      }
    };
    const setRotation = (key: string, idx: number) => {
      ...
      m[key] = idx;
      try {
        localStorage.setItem("__gifRotation", JSON.stringify(m));
      } catch {}
    };
    ...
    (async () => {
      ...
      if (usedOverride) {
        console.log("GIF Debug: Used override");
        return;
      }

      // Try new keywords from the current answer/query first (avoid ones used before)
      const kwFromText = extractKeywords(
        `${intentText} ${text} ${(related || []).map((r) => r.title).join(" ")}`
      );
      ...
      for (const kw of fresh) {
        if (await tryKeyword(kw)) {
          console.log("GIF Debug: Found via fresh keyword:", kw);
          return;
        }
      }
      ...
      for (let i = kwHistory.length - 1; i >= 0; i--) {
        if (await tryKeyword(kwHistory[i])) return;
      }
  ...
  return (
    <div className="answer-gif-wrapper mb-4 px-4">
      <div style={{ position: "relative", display: "inline-block" }}>{img}</div>
    </div>
  );
};
...
const SuggestionsSection: React.FC<{
  ...
  return (
    <div className="mt-6">
      <h5 className="mb-3 px-4 font-semibold text-gray-800">Suggested Questions</h5>
      <div className="px-4">
        {suggestions.map((suggestion, index) => (
          <button
            key={index}
            onClick={() => handleSuggestionClick(suggestion)}
            className={`suggestion-button flex w-full items-center justify-between text-left text-sm ${
...
const renderTables = (answer: string, tables: Table[]): string => {
  ...
  const buildTableHtml = (table: Table) => {
    ...
    (table.rows || []).forEach((row) => {
      ...
      tableHtml += row
        .map((cell) => `<td class=\"p-3 text-sm text-gray-800\">${cell}</td>`)
        .join("");
      tableHtml += `</tr>`;
    });
    tableHtml += `</tbody>`;
...
const getImageNaturalSize = (dataUrl: string): Promise<{ width: number; height: number }> => {
  return new Promise((resolve) => {
    const im = new Image();
    im.onload = () =>
      resolve({ width: im.naturalWidth || im.width, height: im.naturalHeight || im.height });
